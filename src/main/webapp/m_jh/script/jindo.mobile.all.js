...
jindo.$Jindo.mixin = function(oDestination, oSource) {
    ...
    for (i in oSource)
        if (oSource.hasOwnProperty(i) && !jindo.$Jindo.isHash(oSource[i])) {
            oReturn[i] = oSource[i]
        }
    return oReturn
};
...
jindo.$Jindo.checkVarType = function(aArgs, oRules, sFuncName) {
    ...
    for (var sType in oRules)
        if (oRules.hasOwnProperty(sType)) {
            ...
            if (bCompat) {
                aBodyThenCode.push("nMatchScore = " + (nRuleLen * 1e3 + nTypeCount * 10) + " + (nArgsLen === " + nRuleLen + " ? 1 : 0);");
...
jindo.$Jindo.varType = function() {
    var oArgs = this.checkVarType(arguments, {s4str: ["sTypeName:String+", "fpFunc:Function+"],s4obj: ["oTypeLists:Hash+"],g: ["sTypeName:String+"]});
...
(function() {
    ...
    for (var sTypeName in cache)
        if (cache.hasOwnProperty(sTypeName)) {
    ...
    cache.varType("ArrayStyle", function(val, bAutoCast) {
        ...
        if (/(Arguments|NodeList|HTMLCollection|global|Window)/.test(jindo._p_._objToString.call(val)) || /Object/.test(jindo._p_._objToString.call(val)) && cache.isNumeric(val.length)) {
...
jindo._p_._createEle = function(sParentTag, sHTML, oDoc, bWantParent) {
    var sId = "R" + (new Date).getTime() + parseInt(Math.random() * 1e5, 10);
...
jindo.$Class.extend = function(superClass) {
    var oArgs = g_checkVarType(arguments, {"4obj": ["oDef:$Class"]}, "<static> $Class#extend");
    this.prototype._$superClass = superClass;
    var superProto = superClass.prototype;
    for (var prop in superProto) {
        if (jindo.$Jindo.isHash(superProto[prop]))
...
jindo.$$ = jindo.cssquery = function() {
    ...
    function _addQueryId(el, sIdName) {
        ...
        if (/^\w+$/.test(el.id)) {
            ...
            sValue = "C" + (new Date).getTime() + Math.floor(Math.random() * 1e6);
    ...
    function distinct(aList) {
        var aDistinct = [], oDummy = {}, nUID, oEl, i;
    ...
    function makeQueryParseTree(sQuery) {
        ...
        if (match) {
            ...
            returnVal.push({left: sQuery,com: "",right: ""})
        }
        return returnVal
    }
    ...
    cssquery = function(sQuery, oParent, oOptions) {
        var oArgs = jindo.$Jindo.checkVarType(arguments, {"4str": ["sQuery:String+"],"4var": ["sQuery:String+", "oParent:Variant"],"4var2": ["sQuery:String+", "oParent:Variant", "oOptions:Variant"]}, "cssquery"), sTempId, aRet, nParentNodeType, sProperty, oOldParent, queryid, _clone, sTagName, _parent;
...
jindo.$Agent.prototype.os = function() {
    var info = {}, u = this._navigator.userAgent, p = this._navigator.platform, f = function(s, h) {
        return h.indexOf(s) > -1
    }, aMatchResult = null;
    info.getName = function() {
        var name = "";
...
jindo.$A.prototype.set = function(nIndex, vValue) {
    g_checkVarType(arguments, {"4num": ["nIndex:Numeric", "vValue:Variant"]}, "$A#set");
    this._array[nIndex] = vValue;
    return this
};
jindo.$A.prototype.length = function(nLen, oValue) {
    var oArgs = g_checkVarType(arguments, {"4num": [jindo.$Jindo._F("nLen:Numeric")],sv: ["nLen:Numeric", "vValue:Variant"],"4voi": []}, "$A#length");
    switch (oArgs + "") {
        case "4num":
...
jindo.$A.prototype.forEach = function(fCallback, oThis) {
    ...
    try {
        ...
        if (!(e instanceof this.constructor.Break))
            throw e
    }
    return this
};
...
jindo.$A.prototype.sort = function(fpSort) {
    var oArgs = g_checkVarType(arguments, {"void": [],"4fp": ["fpSort:Function+"]}, "$A#sort");
    if (fpSort) {
...
jindo.$A.prototype.filter = function(fCallback, oThis) {
    ...
    function f(v, i, a) {
        try {
            if (fCallback.apply(oThis || that, jindo._p_._toArray(arguments)))
                returnArr.push(v)
        } catch (e) {
            if (!(e instanceof that.constructor.Continue)) {
                throw e
...
jindo.$H.prototype.$ = function(key, value) {
    var oArgs = g_checkVarType(arguments, {s4var: [jindo.$Jindo._F("key:String+"), "value:Variant"],s4var2: ["key:Numeric", "value:Variant"],g4str: ["key:String+"],s4obj: ["oObj:Hash+"],g4num: ["key:Numeric"]}, "$H#$");
    switch (oArgs + "") {
        case "s4var":
        case "s4var2":
...
jindo.$H.prototype.forEach = function(callback, scopeObject) {
    var oArgs = g_checkVarType(arguments, {"4fun": ["callback:Function+"],"4obj": ["callback:Function+", "thisObject:Variant"]}, "$H#forEach");
    var t = this._table;
...
jindo.$H.prototype.filter = function(callback, thisObject) {
    var oArgs = g_checkVarType(arguments, {"4fun": ["callback:Function+"],"4obj": ["callback:Function+", "thisObject:Variant"]}, "$H#filter");
    var h = jindo.$H();
    var t = this._table;
...
jindo.$H.prototype.map = function(callback, thisObject) {
    var oArgs = g_checkVarType(arguments, {"4fun": ["callback:Function+"],"4obj": ["callback:Function+", "thisObject:Variant"]}, "$H#map");
    var h = jindo.$H();
    var t = this._table;
    var hCon = this.constructor;
    for (var k in t) {
...
jindo.$H.prototype.add = function(key, value) {
    var oArgs = g_checkVarType(arguments, {"4str": ["key:String+", "value:Variant"],"4num": ["key:Numeric", "value:Variant"]}, "$H#add");
    var sortedIndex = this["__jindo_sorted_index"];
...
jindo.$H.prototype.remove = function(key) {
    var oArgs = g_checkVarType(arguments, {"4str": ["key:String+"],"4num": ["key:Numeric"]}, "$H#remove");
...
jindo.$H.prototype.search = function(value) {
    var oArgs = g_checkVarType(arguments, {"4str": ["value:Variant"]}, "$H#search");
    var result = false;
    var t = this._table;
    for (var k in t) {
        if (t.hasOwnProperty(k)) {
...
jindo.$H.prototype.hasKey = function(key) {
    var oArgs = g_checkVarType(arguments, {"4str": ["key:String+"],"4num": ["key:Numeric"]}, "$H#hasKey");
    return this._table[key] !== undefined
};
jindo.$H.prototype.hasValue = function(value) {
    var oArgs = g_checkVarType(arguments, {"4str": ["value:Variant"]}, "$H#hasValue");
...
jindo.$H.prototype.sort = function(fpSort) {
    var oArgs = g_checkVarType(arguments, {vo: [],"4fp": ["fpSort:Function+"]}, "$H#sort");
    this["__jindo_sorted_index"] = jindo._p_.defaultSort(oArgs, this, "val");
    return this
};
jindo.$H.prototype.ksort = function(fpSort) {
    var oArgs = g_checkVarType(arguments, {vo: [],"4fp": ["fpSort:Function+"]}, "$H#ksort");
...
jindo.$Fn.prototype.delay = function(nSec, args) {
    var oArgs = g_checkVarType(arguments, {"4num": ["nSec:Numeric"],"4arr": ["nSec:Numeric", "args:Array+"]}, "$Fn#delay");
    switch (oArgs + "") {
        case "4num":
            args = args || [];
            break;
    ...
    this._delayKey = setTimeout(this.bind.apply(this, args), nSec * 1e3);
    return this
};
jindo.$Fn.prototype.setInterval = function(nSec, args) {
    var oArgs = g_checkVarType(arguments, {"4num": ["nSec:Numeric"],"4arr": ["nSec:Numeric", "args:Array+"]}, "$Fn#setInterval");
    switch (oArgs + "") {
        case "4num":
            args = args || [];
            break;
    ...
    this._repeatKey = setInterval(this.bind.apply(this, args), nSec * 1e3);
    return this
};
...
jindo._p_.addCustomEventListener = function(eEle, sElementId, sEvent, vFilter, oCustomInstance) {
    ...
    if (!jindo._p_.customEventStore[sElementId][sEvent][vFilter]) {
        jindo._p_.customEventStore[sElementId][sEvent][vFilter] = {custom: oCustomInstance}
    }
};
...
jindo.$Event.customEvent = function(sName, oEvent) {
    var oArgs = g_checkVarType(arguments, {s4str: ["sName:String+"],s4obj: ["sName:String+", "oEvent:Hash+"]}, "$Event.customEvent");
    switch (oArgs + "") {
        case "s4str":
            ...
            return function(nIndex) {
                if (this.isTouch) {
                    ...
                    this[sType] = function(nIndex) {
                        var oArgs = g_checkVarType(arguments, {"void": [],"4num": ["nIndex:Numeric"]}, "$Event#" + sType);
                        if (oArgs + "" == "void")
                            return oRet;
                        return oRet[nIndex]
                    }
...
jindo._p_._makeRandom = function() {
    return "e" + (new Date).getTime() + parseInt(Math.random() * 1e8, 10)
};
jindo._p_.releaseEventHandlerForAllChildren = function(wel) {
    var children = wel._element.all || wel._element.getElementsByTagName("*"), nChildLength = children.length, elChild = null, i;
...
jindo.$Element.prototype.visible = function(bVisible, sDisplay) {
    var oArgs = g_checkVarType(arguments, {g: [],s4bln: [jindo.$Jindo._F("bVisible:Boolean")],s4str: ["bVisible:Boolean", "sDisplay:String+"]}, "$Element#visible");
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.show = function(sDisplay) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4str": ["sDisplay:String+"]}, "$Element#show");
...
jindo.$Element.prototype.toggle = function(sDisplay) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4str": ["sDisplay:String+"]}, "$Element#toggle");
...
jindo.$Element.prototype.opacity = function(value) {
    var oArgs = g_checkVarType(arguments, {g: [],s: ["nOpacity:Numeric"]}, "$Element#opacity");
...
jindo.$Element.prototype.css = function(sName, sValue) {
    var oArgs = g_checkVarType(arguments, {g: ["sName:String+"],s4str: [jindo.$Jindo._F("sName:String+"), jindo.$Jindo._F("vValue:String+")],s4num: ["sName:String+", "vValue:Numeric"],s4obj: ["oObj:Hash+"]}, "$Element#css");
    var e = this._element;
    switch (oArgs + "") {
        case "s4str":
        case "s4num":
    ...
    for (var k in sName) {
        if (sName.hasOwnProperty(k)) {
            v = sName[k];
            if (!(jindo.$Jindo.isString(v) || jindo.$Jindo.isNumeric(v)))
                continue;
            if (k == "opacity") {
                this.opacity(v);
                continue
...
jindo.$Element.prototype.attr = function(sName, sValue) {
    var oArgs = g_checkVarType(arguments, {g: ["sName:String+"],s4str: ["sName:String+", "vValue:String+"],s4num: ["sName:String+", "vValue:Numeric"],s4nul: ["sName:String+", "vValue:Null"],s4bln: ["sName:String+", "vValue:Boolean"],s4arr: ["sName:String+", "vValue:Array+"],s4obj: [jindo.$Jindo._F("oObj:Hash+")]}, "$Element#attr");
    ...
    fGetIndex = function(oOPtions, vValue) {
        var nIndex = -1, i, length, elOption;
...
jindo.$Element.prototype.width = function(width) {
    var oArgs = g_checkVarType(arguments, {g: [],s: ["nWidth:Numeric"]}, "$Element#width");
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.height = function(height) {
    var oArgs = g_checkVarType(arguments, {g: [],s: ["nHeight:Numeric"]}, "$Element#height");
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.className = function(sClass) {
    var oArgs = g_checkVarType(arguments, {g: [],s: [jindo.$Jindo._F("sClass:String+")]}, "$Element#className");
    var e = this._element;
    switch (oArgs + "") {
        case "g":
            return e.className;
...
jindo.$Element.prototype.hasClass = function(sClass) {
    ...
    if (jindo._p_.canUseClassList()) {
        jindo.$Element.prototype.hasClass = function(sClass) {
            var oArgs = ___checkVarType(arguments, {"4str": ["sClass:String+"]}, "$Element#hasClass");
...
jindo.$Element.prototype.removeClass = function(sClass) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sClass:String+"]}, "$Element#removeClass");
    var e = this._element;
    var sClassName = e.className;
    var aClass = (sClass + "").split(" ");
    var sEachClass;
...
jindo.$Element.prototype.toggleClass = function(sClass, sClass2) {
    ...
    if (jindo._p_.canUseClassList()) {
        jindo.$Element.prototype.toggleClass = function(sClass, sClass2) {
            var oArgs = ___checkVarType(arguments, {"4str": ["sClass:String+"],"4str2": ["sClass:String+", "sClass2:String+"]}, "$Element#toggleClass");
            switch (oArgs + "") {
                case "4str":
...
jindo.$Element.prototype.cssClass = function(vClass, bCondition) {
    var oArgs = g_checkVarType(arguments, {g: ["sClass:String+"],s4bln: ["sClass:String+", "bCondition:Boolean"],s4obj: ["oObj:Hash+"]}, "$Element#cssClass");
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.text = function(sText) {
    var oArgs = g_checkVarType(arguments, {g: [],s4str: ["sText:String+"],s4num: [jindo.$Jindo._F("sText:Numeric")],s4bln: ["sText:Boolean"]}, "$Element#text"), ele = this._element, tag = this.tag, prop, oDoc;
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.html = function(sHTML) {
    var oArgs = g_checkVarType(arguments, {g: [],s4str: [jindo.$Jindo._F("sText:String+")],s4num: ["sText:Numeric"],s4bln: ["sText:Boolean"]}, "$Element#html");
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.attach = function(sEvent, fpCallback) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sEvent:String+", "fpCallback:Function+"],"4obj": ["hListener:Hash+"]}, "$Element#attach");
...
jindo.$Element.prototype.detach = function(sEvent, fpCallback) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sEvent:String+", "fpCallback:Function+"],"4obj": ["hListener:Hash+"]}, "$Element#detach");
    var oSpilt, hListener;
    switch (oArgs + "") {
        case "4str":
            oSpilt = jindo._p_.splitEventSelector(oArgs.sEvent);
...
jindo.$Element.prototype.delegate = function(sEvent, vFilter, fpCallback) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sEvent:String+", "vFilter:String+", "fpCallback:Function+"],"4fun": ["sEvent:String+", "vFilter:Function+", "fpCallback:Function+"]}, "$Element#delegate");
    return this._add("delegate", sEvent, vFilter, fpCallback)
};
jindo.$Element.prototype.undelegate = function(sEvent, vFilter, fpCallback) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sEvent:String+", "vFilter:String+", "fpCallback:Function+"],"4fun": ["sEvent:String+", "vFilter:Function+", "fpCallback:Function+"],group_for_string: ["sEvent:String+", "vFilter:String+"],group_for_function: ["sEvent:String+", "vFilter:Function+"]}, "$Element#undelegate");
...
jindo._p_.mouseTouchPointerEvent = function(sEvent) {
    ...
    if (window.navigator.msPointerEnabled && window.navigator.msMaxTouchPoints > 0) {
        ...
        eventMap = {mousedown: "touchstart",mouseup: "touchend",mousemove: "touchmove",pointerdown: "touchstart",pointerup: "touchend",pointermove: "touchmove"}
    }
    jindo._p_.mouseTouchPointerEvent = function(sEvent) {
...
jindo.$Element.eventManager = function() {
    ...
    return {revisionCallback: function(sType, sEvent, realEvent, fpCallback) {
            ...
            if (aMatch) {
                return {event: aMatch[1].toLowerCase(),group: aMatch[2].toLowerCase()}
            } else {
                return {event: sEvent.toLowerCase(),group: jindo._p_.NONE_GROUP}
            }
...
jindo.$Element.prototype.appear = function(duration, callback) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4num": ["nDuration:Numeric"],"4fun": ["nDuration:Numeric", "fpCallback:Function+"]}, "$Element#appear");
    switch (oArgs + "") {
        case "4voi":
            duration = .3;
            callback = function() {
...
jindo.$Element.prototype.disappear = function(duration, callback) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4num": ["nDuration:Numeric"],"4fun": ["nDuration:Numeric", "fpCallback:Function+"]}, "$Element#disappear");
    switch (oArgs + "") {
        case "4voi":
            duration = .3;
            callback = function() {
...
jindo.$Element.prototype.offset = function(nTop, nLeft) {
    var oArgs = g_checkVarType(arguments, {g: [],s: ["nTop:Numeric", "nLeft:Numeric"]}, "$Element#offset");
    switch (oArgs + "") {
        case "g":
...
jindo.$Element.prototype.offset_set = function(nTop, nLeft) {
    var oEl = this._element;
    var oPhantom = null;
    if (isNaN(parseFloat(this._getCss(oEl, "top"))))
        oEl.style.top = "0px";
    if (isNaN(parseFloat(this._getCss(oEl, "left"))))
        oEl.style.left = "0px";
...
jindo.$Element.prototype.evalScripts = function(sHTML) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sHTML:String+"]}, "$Element#evalScripts");
    var aJS = [];
...
jindo.$Element.prototype.clone = function(bDeep) {
    var oArgs = g_checkVarType(arguments, {"default": [],set: ["bDeep:Boolean"]}, "$Element#clone");
    if (oArgs + "" == "default") {
        bDeep = true
    }
...
jindo.$Element.prototype.parent = function(pFunc, limit) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4fun": ["fpFunc:Function+"],"4nul": ["fpFunc:Null"],for_function_number: ["fpFunc:Function+", "nLimit:Numeric"],for_null_number: ["fpFunc:Null", "nLimit:Numeric"]}, "$Element#parent");
    var e = this._element;
    switch (oArgs + "") {
        case "4voi":
    ...
    while (e.parentNode && limit-- != 0) {
        ...
        if (e.parentNode == document.documentElement)
            break;
        if (!pFunc || pFunc && pFunc.call(this, p))
            a[a.length] = p;
        e = e.parentNode
    }
    return a
...
jindo.$Element.prototype.child = function(pFunc, limit) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4fun": ["fpFunc:Function+"],"4nul": ["fpFunc:Null"],for_function_number: ["fpFunc:Function+", "nLimit:Numeric"],for_null_number: ["fpFunc:Null", "nLimit:Numeric"]}, "$Element#child");
    var e = this._element;
    var a = [], c = null, f = null;
    switch (oArgs + "") {
        case "4voi":
    ...
    (f = function(el, lim, context) {
        ...
        for (var i = 0; i < el.childNodes.length; i++) {
            ...
            if (!pFunc || pFunc && pFunc.call(context, o))
                a[a.length] = o;
            if (lim != 0)
...
jindo.$Element.prototype.prev = function(pFunc) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4fun": ["fpFunc:Function+"],"4nul": ["fpFunc:Null"]}, "$Element#prev");
    var e = this._element;
    var a = [];
    switch (oArgs + "") {
        case "4voi":
...
jindo.$Element.prototype.next = function(pFunc) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4fun": ["fpFunc:Function+"],"4nul": ["fpFunc:Null"]}, "$Element#next");
    var e = this._element;
    var a = [];
    switch (oArgs + "") {
        case "4voi":
...
jindo.$Element.prototype.fireEvent = function(sEvent, oProps) {
    var oArgs = g_checkVarType(arguments, {"4str": [jindo.$Jindo._F("sEvent:String+")],"4obj": ["sEvent:String+", "oProps:Hash+"]}, "$Element#fireEvent");
    var ele = this._element;
    var oldEvent = sEvent;
...
jindo.$Element.prototype.ellipsis = function(stringTail) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4str": ["stringTail:String+"]}, "$Element#ellipsis");
...
jindo.$Element.prototype.queryAll = function(sSelector) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sSelector:String+"]}, "$Element#queryAll");
...
jindo.$Element.prototype.query = function(sSelector) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sSelector:String+"]}, "$Element#query");
...
jindo.$Element.prototype.test = function(sSelector) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sSelector:String+"]}, "$Element#test");
    return jindo.cssquery.test(this._element, sSelector)
};
jindo.$Element.prototype.xpathAll = function(sXPath) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sXPath:String+"]}, "$Element#xpathAll");
...
jindo.$Element.prototype.hasEventListener = function(sEvent) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sEvent:String+"]}, "$Element#hasEventListener"), oDoc, bHasEvent = false, sLowerCaseEvent = oArgs.sEvent.toLowerCase();
    if (this._key) {
...
jindo.$Element.prototype.data = function(sKey, vValue) {
    var jindoKey = "_jindo";
    var sToStr, oArgs = g_checkVarType(arguments, {g: ["sKey:String+"],s4var: ["sKey:String+", "vValue:Variant"],s4obj: ["oObj:Hash+"]}, "$Element#data");
...
jindo.$ElementList.prototype.get = function(idx) {
    var oArgs = g_checkVarType(arguments, {"4num": ["nIdx:Numeric"]}, "$ElementList#get");
...
jindo.$ElementList.prototype.length = function(nLen, oValue) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4num": [jindo.$Jindo._F("nLen:Numeric")],"4var": ["nLen:Numeric", "oValue:Variant"]}, "$ElementList#length");
...
jindo.$Form.prototype.element = function(sKey) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4str": [jindo.$Jindo._F("sKey:String+")]}, "$Form#element");
    switch (oArgs + "") {
        case "4voi":
...
jindo.$Form.prototype.enable = function(sKey) {
    var oArgs = g_checkVarType(arguments, {s4bln: ["sName:String+", "bEnable:Boolean"],s4obj: ["oObj:Hash+"],g: [jindo.$Jindo._F("sName:String+")]}, "$Form#enable");
    switch (oArgs + "") {
        case "s4bln":
...
jindo.$Form.prototype.value = function(sKey) {
    var oArgs = g_checkVarType(arguments, {s4str: ["sKey:String+", "vValue:Variant"],s4obj: [jindo.$Jindo._F("oObj:Hash+")],g: ["sKey:String+"]}, "$Form#value");
    if (oArgs + "" == "s4obj") {
        var self = this;
        sKey = oArgs.oObj;
        for (var k in sKey) {
...
jindo.$Form.prototype.submit = function(sTargetName, fValidation) {
    var oArgs = g_checkVarType(arguments, {voi: [],"4str": ["sTargetName:String+"],"4fun": ["fValidation:Function+"],"4fun2": ["sTargetName:String+", "fValidation:Function+"]}, "$Form#submit");
    var sOrgTarget = null;
    switch (oArgs + "") {
        case "4str":
        ...
        case "4fun":
        case "4fun2":
            if (!oArgs.fValidation.call(this, this._form))
                return this;
            if (oArgs + "" == "4fun2") {
                sOrgTarget = this._form.target;
                this._form.target = oArgs.sTargetName
    ...
    this._form.submit();
    if (!jindo.$Jindo.isNull(sOrgTarget))
        this._form.target = sOrgTarget;
    return this
};
jindo.$Form.prototype.reset = function(fValidation) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4fun": ["fValidation:Function+"]}, "$Form#reset");
    if (oArgs + "" == "4fun")
        if (!fValidation.call(this, this._form))
            return this;
    this._form.reset();
    return this
};
    ...
    for (var i in type) {
        jindo.$Document.prototype[i] = function(sMethod, fp) {
            return function(sQuery) {
                var oArgs = g_checkVarType(arguments, {"4str": ["sQuery:String+"]}, "$Document#" + sMethod);
...
jindo.$S.prototype.times = function(nTimes) {
    var oArgs = g_checkVarType(arguments, {"4str": ["nTimes:Numeric"]}, "$S#times");
    if (!oArgs) {
        return this
    }
...
jindo.$S.prototype.bytes = function(vConfig) {
    var oArgs = g_checkVarType(arguments, {"4voi": [],"4num": ["nConfig:Numeric"],"4obj": ["nConfig:Hash+"]}, "$S#bytes");
...
jindo.$S.prototype.parseString = function() {
    ...
    for (var i = 0; i < str.length; i++) {
        ...
        if (key.substr(key.length - 2, 2) == "[]") {
            ...
            if (jindo.$Jindo.isUndefined(buf[key]))
                buf[key] = [];
...
jindo.$Json.fromXML = function(sXML) {
    ...
    var o = {};
    var re = /\s*<(\/?[\w:\-]+)((?:\s+[\w:\-]+\s*=\s*(?:"(?:\\"|[^"])*"|'(?:\\'|[^'])*'))*)\s*((?:\/>)|(?:><\/\1>|\s*))|\s*<!\[CDATA\[([\w\W]*?)\]\]>\s*|\s*>?([^<]*)/gi;
    var re2 = /^[0-9]+(?:\.[0-9]+)?$/;
    var ec = {"&amp;": "&","&nbsp;": " ","&quot;": '"',"&lt;": "<","&gt;": ">"};
    var fg = {tags: ["/"],stack: [o]};
    var es = function(s) {
        if (cache.isUndefined(s))
            return "";
...
jindo.$Json._oldToString = function(oObj) {
    var cache = jindo.$Jindo;
    var func = {$: function($) {
            if (cache.isNull($) || !cache.isString($) && $ == Infinity)
                return "null";
            if (cache.isFunction($))
                return undefined;
            ...
            if (cache.isDate($))
                return $ + "";
            if (typeof $ == "object" || cache.isRegExp($))
                return "{}";
            if (isNaN($))
                return "null"
        },s: function(s) {
            ...
            for (var x in o) {
                if (o.hasOwnProperty(x)) {
                    if (cache.isUndefined(o[x]) || cache.isFunction(o[x]))
                        continue;
...
jindo.$Ajax = function(url, option) {
    ...
    this._headers = {};
    this._options = {type: "xhr",method: "post",proxy: "",timeout: 0,onload: function(req) {
        },onerror: null,ontimeout: function(req) {
        },jsonp_charset: "utf-8",callbackid: "",callbackname: "",async: true,decode: true,postBody: false,withCredentials: false};
    this._options = ___ajax._setProperties(oArgs.oOption, this);
    if (this._options.type == "xhr") {
        this._url = oArgs.sURL.replace(new RegExp("^" + window.location.protocol + "//" + window.location.host), function() {
            return ""
        })
    }
...
jindo.$Ajax._setProperties = function(option, context) {
    ...
    option.onload = jindo.$Fn(option.onload || function() {
    }, context).bind();
    option.ontimeout = jindo.$Fn(option.ontimeout || function() {
    }, context).bind();
    option.onerror = jindo.$Fn(option.onerror || function() {
    }, context).bind();
...
jindo.$Ajax._validationOption = function(oOption, sMethod) {
    ...
    var oTypeProperty = {xhr: "onload|timeout|ontimeout|onerror|async|method|postBody|type|withCredentials",jsonp: "onload|timeout|ontimeout|onerror|jsonp_charset|callbackid|callbackname|method|type"};
    var aName = [];
    var i = 0;
...
jindo.$Ajax.prototype.request = function(oData) {
    ...
    if (req.addEventListener) {
        ...
        if (this._errorFun) {
            req.removeEventListener("error", this._errorFun, false)
        }
    ...
    if (opt.timeout > 0) {
        ...
        _timer = setTimeout(function() {
            ...
            opt.ontimeout(req);
            if (cache.isFunction(t._oncompleted))
...
jindo.$Ajax.prototype.url = function(sURL) {
    var oArgs = g_checkVarType(arguments, {g: [],s: ["sURL:String+"]}, "$Ajax#url");
    switch (oArgs + "") {
        case "g":
            return this._url;
        case "s":
...
jindo.$Ajax.prototype.option = function(name, value) {
    var oArgs = g_checkVarType(arguments, {s4var: ["sKey:String+", "vValue:Variant"],s4obj: ["oOption:Hash+"],g: ["sKey:String+"]}, "$Ajax#option");
    switch (oArgs + "") {
        case "s4var":
            oArgs.oOption = {};
...
jindo.$Ajax.Response.prototype.header = function(name) {
    var oArgs = g_checkVarType(arguments, {"4str": ["name:String+"],"4voi": []}, "$Ajax.Response#header");
    switch (oArgs + "") {
        case "4str":
...
jindo.$Ajax.RequestBase = klass({_respHeaderString: "",callbackid: "",callbackname: "",responseXML: null,responseJSON: null,responseText: "",status: 404,readyState: 0,$init: function(fpOption) {
    },onload: function() {
    },abort: function() {
    },open: function() {
    },send: function() {
    },setRequestHeader: function(sName, sValue) {
        g_checkVarType(arguments, {"4str": ["sName:String+", "sValue:String+"]}, "$Ajax.RequestBase#setRequestHeader");
        this._headers[sName] = sValue
    },getResponseHeader: function(sName) {
        g_checkVarType(arguments, {"4str": ["sName:String+"]}, "$Ajax.RequestBase#getResponseHeader");
        ...
        if (this.option("callbackid") != "") {
            ...
            do {
                id = "_" + Math.floor(Math.random() * 1e4)
            } while (window["__" + jindo._p_.jindoName + "_callback"][id])
        }
...
jindo.$Ajax.JSONPRequest = klass({_headers: {},_respHeaders: {},_script: null,_onerror: null,$init: function(fpOption) {
        ...
        g_checkVarType(arguments, {"4str": ["method:String+", "url:String+"]}, "$Ajax.JSONPRequest#open");
        this.responseJSON = null;
        this._url = url
    },send: function(data) {
        var oArgs = g_checkVarType(arguments, {"4voi": [],"4nul": ["data:Null"],"4str": ["data:String+"]}, "$Ajax.JSONPRequest#send");
        var t = this;
...
jindo.$Ajax.Queue.prototype.option = function(name, value) {
    var oArgs = g_checkVarType(arguments, {s4str: ["sKey:String+", "sValue:Variant"],s4obj: ["oOption:Hash+"],g: ["sKey:String+"]}, "$Ajax.Queue#option");
    switch (oArgs + "") {
        case "s4str":
...
jindo.$Ajax.Queue.prototype.add = function(oAjax, oParam) {
    var oArgs = g_checkVarType(arguments, {"4obj": ["oAjax:Hash+"],"4obj2": ["oAjax:Hash+", "oPram:Hash+"]}, "$Ajax.Queue");
    switch (oArgs + "") {
        case "4obj2":
            oParam = oArgs.oPram
    }
...
jindo.$Date._paramCheck = function(aPram, sType) {
    return g_checkVarType(aPram, {s: ["nParm:Numeric"],g: []}, "$Date#" + sType)
};
...
jindo.$Date.prototype.name = function(vName, aValue) {
    var oArgs = g_checkVarType(arguments, {s4str: ["sKey:String+", "aValue:Array+"],s4obj: ["oObject:Hash+"],g: ["sKey:String+"]}, "$Date#name");
    switch (oArgs + "") {
        case "s4str":
...
jindo.$Date.parse = function(strDate) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sKey:String+"]}, "$Date#parse");
...
jindo.$Date.prototype.format = function(strFormat) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sFormat:String+"]}, "$Date#format");
    strFormat = oArgs.sFormat;
    var o = {};
    var d = this._date;
    var name = this._names;
    var self = this;
    return (strFormat || "").replace(/[a-z]/gi, function callback(m) {
        if (o[m] !== undefined)
            return o[m];
        switch (m) {
            case "d":
            ...
            case "z":
                o.z = Math.floor((d.getTime() - new Date(d.getFullYear(), 0, 1).getTime()) / (3600 * 24 * 1e3));
                return o.z;
            case "m":
            case "n":
                o.n = d.getMonth() + 1;
...
jindo.$Date.prototype.compare = function(oDate, sType) {
    var oArgs = g_checkVarType(arguments, {"4dat": ["oDate:Date+"],"4str": ["oDate:Date+", "sType:String+"]}, "$Date#compare");
    oDate = oArgs.oDate;
    sType = oArgs.sType;
    if (!sType) {
        return oDate - this._date
    } else if (sType === "s") {
        return Math.floor(oDate / 1e3) - Math.floor(this._date / 1e3)
    } else if (sType === "i") {
        return Math.floor(Math.floor(oDate / 1e3) / 60) - Math.floor(Math.floor(this._date / 1e3) / 60)
    } else if (sType === "h") {
        return Math.floor(Math.floor(Math.floor(oDate / 1e3) / 60) / 60) - Math.floor(Math.floor(Math.floor(this._date / 1e3) / 60) / 60)
    } else if (sType === "d") {
        return Math.floor(Math.floor(Math.floor(Math.floor(oDate / 1e3) / 60) / 60) / 24) - Math.floor(Math.floor(Math.floor(Math.floor(this._date / 1e3) / 60) / 60) / 24)
    } else if (sType === "m") {
...
jindo.$Cookie = function() {
    ...
    if (!(this instanceof cl))
        return new cl;
    if (typeof jindo.$Jindo.isUndefined(cl._cached))
        cl._cached = this;
...
jindo.$Cookie.prototype.get = function(sName) {
    var oArgs = jindo.$Jindo.checkVarType(arguments, {"4str": ["sName:String+"]}, "$Cookie#get");
...
jindo.$Cookie.prototype.set = function(sName, sValue, nDays, sDomain, sPath) {
    ...
    if (oArgs + "" !== "4str" && nDays !== 0) {
        sExpire = ";expires=" + new Date((new Date).getTime() + nDays * 1e3 * 60 * 60 * 24).toGMTString()
    }
    if (cache.isUndefined(sDomain))
        sDomain = "";
    if (cache.isUndefined(sPath))
        sPath = "/";
...
jindo.$Cookie.prototype.remove = function(sName, sDomain, sPath) {
    ...
    if (!cache.isNull(this.get(sName)))
        this.set.apply(this, aPram);
    return this
};
jindo.$Template = function(str, sEngineName) {
    var obj = null, tag = "", cl = arguments.callee, _sEngineName;
    if (str instanceof cl) {
        return str
    }
    if (!(this instanceof cl)) {
...
jindo.$Template.addEngine = function(sEngineName, fEngine) {
    var oArgs = g_checkVarType(arguments, {"4fun": ["sEngineName:String+", "fEngine:Function+"]}, "$Template#addEngine");
    jindo.$Template._aEngines[oArgs.sEngineName] = oArgs.fEngine
};
jindo.$Template.getEngine = function(sEngineName) {
    var oArgs = g_checkVarType(arguments, {"4str": ["sEngineName:String+"]}, "$Template#getEngine");
    return jindo.$Template._aEngines[oArgs.sEngineName]
};
jindo.$Template.prototype.process = function(data) {
    var oArgs = g_checkVarType(arguments, {"4obj": ["data:Hash+"],"4voi": []}, "$Template#process"), fProcess;
...
jindo.$Template.addEngine("default", function(sTemplate) {
    var key = "", leftBrace = "", rightBrace = "", tpl = (" " + sTemplate + " ").replace(/\\{/g, leftBrace).replace(/\\}/g, rightBrace).replace(/(?!\\)\}\{/g, "}" + key + "{").split(jindo.$Template.splitter), i = tpl.length, map = {'"': '\\"',"\\": "\\\\","\n": "\\n","\r": "\\r","	": "\\t","\f": "\\f"}, reg = [/(["'](?:(?:\\.)+|[^\\["']+)*["']|[a-zA-Z_](?:[\w\.]|\[(?:.*?)\])*)/g, /[\n\r\t\f"\\]/g, /^\s+/, /\s+$/, /#/g], cb = [function(m) {
...
jindo.$Template.addEngine("micro", function(sTemplate) {
    return new Function("obj", "var p=[],print=function(){p.push.apply(p,arguments);};" + "with(obj){p.push('" + sTemplate.replace(/[\r\t\n]/g, " ").split("<%").join("	").replace(/((^|%>)[^\t]*)'/g, "$1\r").replace(/\t=(.*?)%>/g, "',$1,'").split("	").join("');").split("%>").join("p.push('").split("\r").join("\\'") + "');}return p.join('');")
});
    ...
    if (jindo[_className]) {
        jindo[_className].hook = function(className) {
            ...
            return function(sName, vRevisionKey) {
                var oArgs = jindo.$Jindo.checkVarType(arguments, {g: ["sName:String+"],s4var: ["sName:String+", "vRevisionKey:Variant"],s4obj: ["oObj:Hash+"]}, "jindo." + className + ".hook");
                switch (oArgs + "") {
                    case "g":
